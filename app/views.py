...
from app.requests import get_root_categories, get_user_by_id, get_user_by_email,get_user_favourites_file, get_file_by_id, user_has_notifications, add_to_user_favourites, remove_from_user_favourites, add_administrator_signalement
...
def search():
    if request.method == 'POST':
        ...
        return render_template('search.html', nom_page=search_term, categorie=get_root_categories()[0], liste_fichiers=get_user_favourites_file(current_user.get_id()), notification_enabled=user_has_notifications(current_user.get_id()))
